...
Каждая сессия и миграция оставляет лог. Логи хранятся локально в `.codex/logs/`, никуда не отправляются, и нужны для анализа ошибок фреймворка.

## Что логируется

### Сессии (`.codex/logs/sessions/`)

При каждом `/start` и `/finish` создаётся/обновляется лог текущей сессии:

**Файл:** `.codex/logs/sessions/YYYY-MM-DD_HH-MM.md`
...
### Миграции (`.codex/logs/migrations/`)

При каждом запуске `migrate.sh`:

**Файл:** `.codex/logs/migrations/YYYY-MM-DD_HH-MM.json`
...
- Для анализа проблем фреймворка: читать `.codex/logs/errors/`
- Для хронологии работы: читать `.codex/logs/sessions/`
