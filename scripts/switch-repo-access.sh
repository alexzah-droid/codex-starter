...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if [ "$TARGET_MODE" = "public" ] || [ "$TARGET_MODE" = "private-shared" ]; then
        if git rev-parse --abbrev-ref --symbolic-full-name '@{u}' >/dev/null 2>&1; then
            if git log --oneline '@{u}' -- .codex CODEx.md manifest.md AGENTS.md 2>/dev/null | grep -q .; then
        ...
        git rm -r --cached --ignore-unmatch .codex CODEx.md manifest.md AGENTS.md >/dev/null 2>&1 || true
    fi
